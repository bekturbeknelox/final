...
import prodimg from "../../Components/ProductCardItem/img/prod1.jpg";
    ...
    try {
      ...
      if (storedCartProduct) {
        ...
        for (let i = 0; i < storedCartProduct.length; i++) {
          ...
          productsInCartTotal +=
            storedCartProduct[i].amountInCart * storedCartProduct[i].price;
      ...
      <Container>
        {cartProducts.length !== 0 ? (
          <div className="cart__title">
            <h1>Корзина</h1>{" "}
            <div onClick={clearCart} className="cart__clear-button">
              Очистить корзину
            </div>
          </div>
        ) : null}
            ...
            <div className="cart__products-list">
              ...
              {cartProducts.map((product) => (
                <CartProduct
                  ...
                  id={product.id}
                  img={prodimg}
                  amountInCartFromServer={product.amountInCart}
                  key={product.id}
                  title={product.title}
                  blend={product.blend}
                  price={product.price}
